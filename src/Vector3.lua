...
function Vector3:init(...)
    ...
    if t then
        if type(t) == "vector3" then
            ...
            local t = {...}
            self.x = tonumber(t[1] or 0)
            self.y = tonumber(t[2] or 0)
            self.z = tonumber(t[3] or 0)
        end
    else
        self.x = 0
        self.y = 0
...
Vector3.Normalize2 = function(vec)
...
Vector3.Add = function(left, right)
    assert(
        (type(left) == "table" and left.type and left.type == "class<vector3>") or type(left) == "vector3",
        "Invalid Vector3 class"
    )
    assert(
        (type(right) == "table" and right.type and right.type == "class<vector3>") or type(right) == "vector3",
        "Invalid Vector3 class"
    )
...
Vector3.Lerp = function(start, _end, amount)
    assert(
        (type(start) == "table" and start.type and start.type == "class<vector3>") or type(start) == "vector3",
        "Invalid Vector3 class"
    )
    assert(
        (type(_end) == "table" and _end.type and _end.type == "class<vector3>") or type(_end) == "vector3",
        "Invalid Vector3 class"
    )
...
Vector3.Reflect = function(vector, normal)
    assert(
        (type(vector) == "table" and vector.type and vector.type == "class<vector3>") or type(vector) == "vector3",
        "Invalid Vector3 class"
    )
    assert(
        (type(normal) == "table" and normal.type and normal.type == "class<vector3>") or type(normal) == "vector3",
        "Invalid Vector3 class"
    )
...
function Vector3:Equals(a)
    assert(
        (type(a) == "table" and a.type and a.type == "class<vector3>") or type(a) == "vector3",
        "Invalid Vector3 class"
    )
...
Vector3.RotationToDirection = function(rotation)
    assert(
        (type(rotation) == "table" and rotation.type and rotation.type == "class<vector3>") or
...
Vector3.DirectionToRotation = function(direction)
    assert(
        (type(direction) == "table" and direction.type and direction.type == "class<vector3>") or
...
function Vector3:toNative()
    return vector3(self.x or 0, self.y or 0, self.z or 0)
end
