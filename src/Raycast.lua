...
RaycastResult.Raycast = function(source, target, options, ignoreEntity)
    local ret = StartExpensiveSynchronousShapeTestLosProbe(source.x, source.y, source.z, target.x, target.y, target.z, options, ignoreEntity.getHandle(), 7)
...
RaycastResult.RaycastCapsule = function(source, target, radius, options, ignoreEntity)
    local ret = StartShapeTestCapsule(source.x, source.y, source.z, target.x, target.y, target.z, radius, options, ignoreEntity.getHandle(), 7)
    return RaycastResult(ret)
end
